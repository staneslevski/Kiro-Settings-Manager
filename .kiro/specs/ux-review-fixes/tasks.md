...
- [ ] 2. CLI Structure — Parser restructuring

  - [ ] 2.1 KsmArgumentParser and core parser refactor

    - [ ] 2.1.1 Create `KsmArgumentParser` in `cli.py` with `error()` override for typo suggestions
      - Import and call `suggest_command()` on unknown command errors
      - Always include `ksm --help` hint in error messages
      - _Requirements: 24.1, 24.2, 24.3, 24.4_

    - [ ] 2.1.2 Add mutually exclusive scope groups for `-l`/`-g` on `add` and `rm`
      - Use `add_mutually_exclusive_group()` in `_build_parser()`
      - _Requirements: 27.1, 27.2, 27.3_

    - [ ] 2.1.3 Add global `--verbose`/`-v` and `--quiet`/`-q` as mutually exclusive group on top-level parser
      - _Requirements: 28.1, 28.2, 28.5_

    - [ ] 2.1.4 Replace `--skills-only`, `--agents-only`, `--steering-only`, `--hooks-only` with repeatable `--only <type>` flag
      - Use `choices=["skills", "agents", "steering", "hooks"]`, `action="append"`
      - Update `_build_subdirectory_filter()` in `commands/add.py`
      - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5_

    - [ ] 2.1.5 Rename `--display` to `--interactive`/`-i` on `add` and `rm`, keep `--display` as hidden alias with deprecation warning
      - _Requirements: 11.1, 11.2, 11.3_

    - [ ] 2.1.6 Write tests for parser changes
  ...
  - [ ] 2.2 Registry subcommand group

    - [ ] 2.2.1 Restructure parser: add `registry` subcommand with `add`, `ls`, `rm`, `inspect` subparsers; remove top-level `add-registry`
      - _Requirements: 4.1, 4.2, 4.3, 4.4_

    - [ ] 2.2.2 Create `commands/registry_add.py` (refactor from `commands/add_registry.py`), update dispatch in `cli.py`
      - Print success to stderr (Req 32.2)
      - _Requirements: 4.2, 32.2_

    - [ ] 2.2.3 Write tests for registry subcommand structure
      - Parser accepts `registry add/ls/rm/inspect`, rejects `add-registry`
      - `registry` with no subcommand shows help
      - **Validates: Requirements 4.1, 4.3, 4.4**

  - [ ] 2.3 Help text and curated help

    - [ ] 2.3.1 Add `RawDescriptionHelpFormatter` and `epilog` with 2-3 examples to every subparser (add, rm, ls, sync, registry add/ls/rm/inspect, init, info, search, completions)
      - Add footer to top-level parser: `Use "ksm <command> --help" for more info`
      - _Requirements: 23.1, 23.2, 23.3_

    - [ ] 2.3.2 Replace default `parser.print_help()` with curated help screen when no command given
      - Show tool name, version, grouped commands, quick-start examples
      - _Requirements: 16.1, 16.2, 16.3_

    - [ ] 2.3.3 Write tests for help text
...
- [ ] 3. Error UX — Enriched error classes, actionable messages

  - [ ] 3.1 Error class enhancements

    - [ ] 3.1.1 Extend `BundleNotFoundError` to accept `searched_registries` list; format message with name, registries, and suggestions
      - _Requirements: 7.1, 7.3_

    - [ ] 3.1.2 Extend `GitError` to accept `url` and `stderr_output`; format cleaned single-line message with URL and suggestion
      - _Requirements: 7.2, 7.4_

    - [ ] 3.1.3 Update `resolver.py` to pass searched registry names to `BundleNotFoundError`
      - _Requirements: 7.1_

    - [ ] 3.1.4 Write property tests for error classes
  ...
  - [ ] 3.2 Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Safety & Feedback — rm confirmation, rm feedback, TTY checks, stderr messages

  - [ ] 4.1 rm confirmation and feedback

    - [ ] 4.1.1 Add `--yes`/`-y` flag to `rm` parser, add `--dry-run` to `add`/`rm`/`sync` parsers
      - _Requirements: 1.4, 12.1, 12.2, 12.3_

    - [ ] 4.1.2 Implement confirmation prompt in `run_rm()`: TTY check, format prompt with bundle name/scope/files, handle y/n/EOF
      - _Requirements: 1.1, 1.2, 1.3, 1.5, 1.6, 31.2_

    - [ ] 4.1.3 Implement removal feedback: format and print `RemovalResult` summary to stderr
      - _Requirements: 2.1, 2.2, 2.3_

    - [ ] 4.1.4 Write property tests for rm confirmation and feedback
  ...
  - [ ] 4.2 Sync TTY check and specific confirmation

    - [ ] 4.2.1 Add TTY check to `run_sync()` confirmation; build specific message with bundle names and file counts
      - _Requirements: 13.1, 13.2, 31.1_

    - [ ] 4.2.2 Write property tests for sync confirmation
  ...
  - [ ] 4.3 Dry-run mode
    ...
    - [ ] 4.3.1 Implement `--dry-run` logic in `run_add()`, `run_rm()`, `run_sync()` — print preview without modifying filesystem/manifest
      - _Requirements: 12.1, 12.2, 12.3, 12.4_

    - [ ] 4.3.2 Write property test for dry-run
      - **Property 15: Dry-run does not modify state**
      - **Validates: Requirements 12.1, 12.2, 12.3, 12.4**

  - [ ] 4.4 Informational messages to stderr

    - [ ] 4.4.1 Move `ls` empty-list message to stderr; move `registry add` success to stderr; audit all commands for stdout-only data
      - _Requirements: 32.1, 32.2, 32.3_

    - [ ] 4.4.2 Write tests for stderr routing
      - **Property 37: Informational messages go to stderr not stdout**
      - **Validates: Requirements 32.1, 32.2, 32.3**

  - [ ] 4.5 Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Interactive Selectors — Headers, stderr, alternate buffer, fallback, filter, multi-select

  - [ ] 5.1 Selector rendering to stderr and alternate screen buffer
  ...
  - [ ] 5.2 Cross-platform fallback and TERM=dumb
  ...
  - [ ] 5.3 Headers, type-to-filter, and multi-select
...
- [ ] 6. Output Quality — ls improvements, CopyResult, file-level diff, auto-launch selector
  ...
  - [ ] 6.3 File-level diff and auto-launch selector
...
- [ ] 7. New Commands — Registry ls/rm/inspect, init, info, search, completions, versioned install
  ...
  - [ ] 7.1 Registry management commands
  ...
  - [ ] 7.3 Completions and versioned install
  ...
  - [ ] 7.4 Wire all new commands into `cli.py` dispatch table
...
- [ ] 8. Cleanup & Polish — Empty dir cleanup, signal handler integration, final validation
  ...
  - [ ] 8.2 Signal handler integration
